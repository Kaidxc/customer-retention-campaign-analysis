...
GitHub supports LaTeX-style math in Markdown, using inline math such as
`$\theta$` and display math such as `$$ ... $$`.
...
$$
\theta = (\alpha_1,\ldots,\alpha_I,\beta_1,\ldots,\beta_I),
\qquad d = \dim(\theta) = 2I.
$$

The current matched-normal prior is:
...
For a target posterior density $\pi(z)$, the unadjusted Langevin update is:
...
$$
\mu \sim \mathcal{N}(m_0, s_\mu^2 I_d),
$$

where $\sigma^2$ is fixed. If we keep the same scale as the current matched-normal
setting, then:
...
\propto
p(H_t \mid \theta)
\mathcal{N}(\theta \mid \mu, \sigma^2 I_d)
\mathcal{N}(\mu \mid m_0, s_\mu^2 I_d).
$$

The log posterior, up to constants, is:

$$
\log p(\theta, \mu \mid H_t)
=
\ell(\theta; H_t)
- \frac{1}{2\sigma^2}\|\theta-\mu\|^2
- \frac{1}{2s_\mu^2}\|\mu-m_0\|^2.
$$
...
+ h
\left[
\nabla_\theta \ell(\theta^{(k)}; H_t)
- \frac{\theta^{(k)}-\mu}{\sigma^2}
\right]
+ \sqrt{2h}\,\xi_\theta^{(k)},
$$
...
$$
\xi_\theta^{(k)} \sim \mathcal{N}(0, I_d).
$$
...
$$
\theta_j \mid \mu_j, \sigma^2 \sim \mathcal{N}(\mu_j, \sigma^2),
\qquad
\mu_j \sim \mathcal{N}(m_{0,j}, s_\mu^2).
$$
...
\mu_j \mid \theta_j
\sim
\mathcal{N}(m_{\text{post},j}, v_{\text{post}}),
$$
...
If all $\alpha_i$ share one unknown mean $\mu_\alpha$, and all $\beta_i$ share
one unknown mean $\mu_\beta$, then:

$$
\alpha_i \mid \mu_\alpha, \sigma_\alpha^2
\sim
\mathcal{N}(\mu_\alpha, \sigma_\alpha^2),
\qquad
\mu_\alpha
\sim
\mathcal{N}(m_{0,\alpha}, s_{\alpha,\mu}^2).
$$
...
\mu_\alpha \mid \alpha
\sim
\mathcal{N}(m_{\alpha,\text{post}}, v_{\alpha,\text{post}}),
$$
...
=
\left(
\frac{1}{s_{\alpha,\mu}^2}
+ \frac{I}{\sigma_\alpha^2}
\right)^{-1},
$$
...
v_{\alpha,\text{post}}
\left(
\frac{m_{0,\alpha}}{s_{\alpha,\mu}^2}
+ \frac{\sum_{i=1}^I \alpha_i}{\sigma_\alpha^2}
\right).
$$
...
v_{\beta,\text{post}}
\left(
\frac{m_{0,\beta}}{s_{\beta,\mu}^2}
+ \frac{\sum_{i=1}^I \beta_i}{\sigma_\beta^2}
\right).
$$
...
1. Given the observed history $H_t$, current $\theta$, current $\mu$, fixed
   $\sigma^2$, and hyper-prior parameters $m_0, s_\mu^2$, repeat for
   $k=1,\ldots,N_t$.
...
\sigma^2
\sim
\operatorname{Inverse\text{-}Gamma}(a,b),
$$
...
$$
p(\theta, \sigma^2 \mid H_t)
\propto
p(H_t \mid \theta)
\mathcal{N}(\theta \mid \mu, \sigma^2 I_d)
\operatorname{Inverse\text{-}Gamma}(\sigma^2 \mid a,b).
$$
...
\sigma^2 \mid \theta
\sim
\operatorname{Inverse\text{-}Gamma}(a_{\text{post}}, b_{\text{post}}),
$$
...
$$
\sigma_\alpha^2 \sim \operatorname{Inverse\text{-}Gamma}(a_\alpha,b_\alpha),
\qquad
\sigma_\beta^2 \sim \operatorname{Inverse\text{-}Gamma}(a_\beta,b_\beta).
$$

The model is:

$$
\alpha_i \mid \mu_{\alpha,i}, \sigma_\alpha^2
\sim
\mathcal{N}(\mu_{\alpha,i}, \sigma_\alpha^2),
$$

$$
\beta_i \mid \mu_{\beta,i}, \sigma_\beta^2
\sim
\mathcal{N}(\mu_{\beta,i}, \sigma_\beta^2).
$$

The Gibbs updates are:

$$
\sigma_\alpha^2 \mid \alpha
\sim
\operatorname{Inverse\text{-}Gamma}
\left(
a_\alpha+\frac{I}{2},
b_\alpha+\frac{1}{2}\sum_{i=1}^I
(\alpha_i-\mu_{\alpha,i})^2
\right),
$$
...
$$
\sigma_\beta^2 \mid \beta
\sim
\operatorname{Inverse\text{-}Gamma}
\left(
a_\beta+\frac{I}{2},
b_\beta+\frac{1}{2}\sum_{i=1}^I
(\beta_i-\mu_{\beta,i})^2
\right).
$$
...
=
\nabla_\alpha \ell(\theta; H_t)
- \frac{\alpha-\mu_\alpha}{\sigma_\alpha^2}.
$$
...
=
\nabla_\beta \ell(\theta; H_t)
- \frac{\beta-\mu_\beta}{\sigma_\beta^2}.
$$
...
\theta \mid \mu, \tau
\sim
\mathcal{N}(\mu, \tau^{-1}I_d),
$$

and:

$$
\tau \sim \operatorname{Gamma}(a,b),
$$

where $\operatorname{Gamma}(a,b)$ uses the rate parameterisation:
...
1. Given observed history $H_t$, current $\theta$, fixed $\mu$, current
   $\sigma^2$, and hyper-prior parameters $a,b$, repeat for
   $k=1,\ldots,N_t$.
...
For separate alpha and beta variances, replace step 3 by:
...
\left(
a_\alpha+\frac{I}{2},
b_\alpha+\frac{1}{2}\|\alpha-\mu_\alpha\|^2
\right),
$$
...
\left(
a_\beta+\frac{I}{2},
b_\beta+\frac{1}{2}\|\beta-\mu_\beta\|^2
\right).
$$
...
| unknown mean + unknown variance | $\mu$, $\sigma^2$ | $\theta$ | $\mu \mid \theta,\sigma^2$; $\sigma^2 \mid \theta,\mu$ |
...
$$
\mu_\alpha = \text{fixed prior mean},
\qquad
\mu_\beta = \text{fixed prior mean}.
$$
...
$$
\tau_\alpha = \frac{1}{\sigma_\alpha^2},
\qquad
\tau_\beta = \frac{1}{\sigma_\beta^2}.
$$
